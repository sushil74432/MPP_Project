...
public class AddNewBookCopyPanel extends JFrame  implements LibWindow{
	...
	public void init() {
		...
		JLabel northLabel = new JLabel("Add New Copy");
		northLabel.setFont(new Font("Fira Code Retina", Font.BOLD, 20));
		northPanel.add(northLabel);
		northPanel.setBackground(ColorPalatte.BACKGROUND);
		...
		txtISBN 	= getLblAndTxt(listPanel, "ISBN", true);
		txtTitle 	= getLblAndTxt(listPanel, "Title",false);
		txtAuthor 	= getLblAndTxt(listPanel, "Authors",false);
		txtCheckoutLeng = getLblAndTxt(listPanel, "Checkout Length",false);
		txtCopies 	= getLblAndTxt(listPanel, "Available Copies",false);
		
		JLabel label = new JLabel("");
		listPanel.add(label);
		
		JPanel AddPanel = new JPanel();
		AddPanel.setBackground(ColorPalatte.BACKGROUND);
		...
		btnAddCopy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane.showMessageDialog(parentFrame,"New book copy has been added!");
					updateBook(b);	
				} catch (Exception e1) {
	...
	private JTextField getLblAndTxt(JPanel parentPanel, String strLbl, boolean bEnable) {
		...
		lblPanel.setBackground(ColorPalatte.BACKGROUND);
		...
		txtPanel.setBackground(ColorPalatte.BACKGROUND);
